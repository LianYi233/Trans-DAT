...
class ToGrayscale(TransformBase):
    ...
    def __init__(self, probability=0.5, use_hsi=False):
        super().__init__()
    ...
    def transform_image(self, image, do_grayscale=False, use_hsi=True):
        use_hsi = self.use_hsi
        # if do_grayscale:
        if torch.is_tensor(image):
            raise NotImplementedError('Implement torch variant.')
        # img_gray = cv.cvtColor(image, cv.COLOR_RGB2GRAY)
        img_gray = cv.cvtColor(image[:, :, 0:3], cv.COLOR_RGB2GRAY)
        if use_hsi:
            hs1 = image[:, :, 3]
            hs2 = image[:, :, 4]
            hs3 = image[:, :, 5]
            return np.stack([img_gray, img_gray, img_gray, hs1, hs2, hs3], axis=2)
        else:
