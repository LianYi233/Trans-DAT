...
class TranstActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        """
        import time
        outputs = self.net(data['search_images'], data['template_images'], data['data_type'], data['alpha'])
        ...
        # print(outputs['domain_search'])

        if len(os.environ['CUDA_VISIBLE_DEVICES']) == 1:
            loss_dict['domain_search'] = outputs['domain_search']             # single gpu
            loss_dict['domain_template'] = outputs['domain_template']
        else:
            loss_dict['domain_search'] = sum(outputs['domain_search'])  # multi-gpu
            loss_dict['domain_template'] = sum(outputs['domain_template'])
