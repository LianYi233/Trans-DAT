...
class Backbone(BackboneBase):
    ...
    def __init__(self,
                 ...
                 use_hsi=False):
        if use_hsi:
            backbone = backbones.resnet50_cbam(output_layers=output_layers, pretrained=pretrained)
        else:
...
def build_backbone(settings, backbone_pretrained=True, frozen_backbone_layers=()):
    ...
    try:
        backbone = Backbone(output_layers=['layer3'], pretrained=backbone_pretrained, frozen_layers=frozen_backbone_layers, use_hsi=settings.env.use_hsi)
        print('w')
    except:
        backbone = Backbone(output_layers=['layer3'], pretrained=backbone_pretrained, frozen_layers=frozen_backbone_layers)
        print('wo')
    # backbone = Backbone(output_layers=['layer3'], pretrained=backbone_pretrained, frozen_layers=frozen_backbone_layers)     # default
