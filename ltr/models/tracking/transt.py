...
class DomainClassifier(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(1024, 1024, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, data, alpha):
        ...
        out2 = self.pool(out1)
        out2 = out2.view(-1, 1024)

        out2 = ReverseLayerF.apply(out2, alpha)
...
class TransT(nn.Module):
    ...
    def forward(self, search, template, domain_target, alpha):
        ...
        domain_search = self.domain(feature_search[0].tensors, alpha)
        feature_template, pos_template = self.backbone(template)
        domain_template = self.domain(feature_template[0].tensors, alpha)
        ...
        loss_domain_search = self.ce_loss_fn(domain_search, domain_target)
        loss_domain_template = self.ce_loss_fn(domain_template, domain_target)
        out = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1],
               'domain_search': loss_domain_search, 'domain_template': loss_domain_template}
        # out = {'pred_boxes': [2,2,3,3]}
        return out
...
def transt_loss(settings):
    ...
    # todo: set the domain loss weight.
    weight_dict = {'loss_ce': 8.334, 'loss_bbox': 5, 'domain_search': 2, 'domain_template': 2}
