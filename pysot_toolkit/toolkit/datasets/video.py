...
class Video(object):
    def __init__(self, name, root, video_dir, init_rect, img_names,
        ...
        if load_img:
            ...
            self.imgs = [cv2.imread(x.split('.')[0]+'.jpg') for x in self.img_names]
            self.width = self.imgs[0].shape[1]
            self.height = self.imgs[0].shape[0]
        else:
            img = cv2.imread(self.img_names[0].split('.')[0]+'.jpg')
    ...
    def __iter__(self):
        for i in range(len(self.img_names)):        # testing phase loads img here
            if self.imgs is not None:
                yield self.imgs[i], self.gt_traj[i]
            else:
                img_fc = cv2.imread(self.img_names[i])  # default: false-color only
                self.use_hsi = True     # IMPORTANT!
                # print('video name:', self.name, ' use_hsi:', self.use_hsi)
                if self.use_hsi:
                    hsi_path = self.img_names[i].replace('-FalseColor', '')
                    hsi_path = hsi_path.replace('jpg', 'png')
                    img_hsi = cv2.imread(hsi_path)
