...
class Tracker(object):
    ...
    def initialize(self, image, info: dict, video_name, use_hsi=False) -> dict:
        ...
        z_crop_hsi = self.get_subwindow(image[:, :, 3:], self.center_pos,
                                    self.exemplar_size,
                                    s_z, self.channel_average)
        # z_crop_save = z_crop[0, 0:3, :, :].cpu().numpy()
        # z_crop_save = np.transpose(z_crop_save, (1, 2, 0))
        # normalize
        z_crop = z_crop.float().mul(1.0 / 255.0).clamp(0.0, 1.0)
        self.mean = [0.485, 0.456, 0.406]
        self.std = [0.229, 0.224, 0.225]
        if use_hsi:
            self.mean = [0.485, 0.456, 0.406, 0.252, 0.184, 0.140]
            self.std = [0.229, 0.224, 0.225, 0.782, 0.590, 0.454]
        self.inplace = False
        z_crop[0] = tvisf.normalize(z_crop[0], self.mean, self.std, self.inplace)

        # initialize template feature
        zf = self.net.template(z_crop)
